  ...
  public async calculateEndDate(startDate: number, subscriptionCycleId: number): Promise<number> {
    try {
      ...
      switch (subscriptionCycle.cycle) {
        case ESubscriptionCycle.MOIS:
          endDate.setMonth(endDate.getMonth() + subscriptionCycle.duration);
          break;
        case ESubscriptionCycle.TRIMESTRE:
          endDate.setMonth(endDate.getMonth() + 3 * subscriptionCycle.duration);
          break;
        case ESubscriptionCycle.SEMESTRE:
          endDate.setMonth(endDate.getMonth() + 6 * subscriptionCycle.duration);
          break;
        case ESubscriptionCycle.ANNUEL:
          endDate.setFullYear(endDate.getFullYear() + subscriptionCycle.duration);
          break;
        default:
          throw new CustomError(400, 'Invalid subscription cycle');
      }
